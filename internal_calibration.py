...
from ..dat.MassAttenuationTables import mass_attenuation_tables
import pandas as pd
import numpy as np
...
class InternalCalibration(StandardCalibration):
    ...
    def __init__(self,
                 adipose_hu=0, air_hu=0, bone_hu=0, muscle_hu=0, label_list=(91, 92, 94, 95)):
    ...
    def _subset(self, all_samples):
        ...
        if 94 in self._label_list:
            sample_subset.append(all_samples[2])  # bone
        if 95 in self._label_list:
            sample_subset.append(all_samples[3])  # muscle
